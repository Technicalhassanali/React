...
export default function RecommendationCar() {
  ...
  const handleViewMore = ()=>{
    ...
    setVisibleItem((previusValue)=>{
     return previusValue+4;
    });
    console.log(visibleItem)
  }
  return ( 
    <div className="recommended-car">
      ...
      <div className="recommended-grid">
        {data.slice(0,visibleItem).map((current, index) => {
          return (
      ...
      {data.length>visibleItem?<div style={{display:"flex",justifyContent:"center"}}>
        <button className="viewMore-btn" onClick={handleViewMore}>
          View More
        </button>
      </div>:<div style={{display:"flex",justifyContent:"center"}}>No More Car</div>}
    </div>
  );
}
